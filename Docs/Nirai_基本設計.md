...
- 頭脳が作業できるのは**許可ディレクトリのリストの中だけ**。初期は専用の作業場のみで、Masterがリストに追記して対象（他プロジェクト等）を広げていく。ただしNirai内部状態の`runtime`とその配下は外部Project扱いせず、明示登録されてもnamed targetへ選べない。通常Agentが`runtime`配下をcwdにできるのは、そのTask自身の`runtime/workspace/<task_id>`だけとする。
- 外部Projectをnamed targetにした場合は、依頼受理時だけでなくQueue復旧・Task Flow開始・相談終了後のProvider起動直前にも対象Folderの実在性と同一性を再検証し、削除・移動・差替えがあればFolderを再生成せず失敗終了する。Provider開始後の承認済みFile反映でも外部Project RootそのものはNirai側で生成せず、Root消失時はfail-closedする。必要な子DirectoryだけはRootが現存する間にその配下へ作成できる。
