...
- Provider Quota / Rate Limit / 明示的な`resource_exhausted`による`interrupted`はCrash由来と区別し、`interruption_reason=provider_quota_exhausted|provider_rate_limit|provider_resource_exhausted`をDurable保存する。リソース不足だけで課金枠の消費とは断定しない。ProviderとFile workerを停止し、Partial Work保存とSession固有cleanupが収束するまでWorkspace Resourceを保持し、収束後に解放してCommanderへ制御を返す。別Residentへ自動reroute・自動再実行しない
- Coreが受理しなかったProviderの状態通知は、保存・配信する`run_state.state`もCoreの確定状態へ揃え、元の申告は`provider_reported_state`へ残す。確認待ちや取消、未確定の完了をTask/UI側で上書きしない。Task状態取得はAgent正本を優先し、`interruption_reason / partial_work_path`をEvent全件読込なしで返す
...
- Holo所有の通常TaskではSocket送信成功をRenderer保存成功とみなさない。World再接続時はownerが残る未回復の終端SessionからTask Updateを再構成し、`interrupted`も再送する。Hostの既存の重複キー・owner削除規則で配送を収束させ、旧回復元Sessionの再開通知は出さない
- Agent Session開始前のTask Queueは`runtime\task_queue.json`を正本とし、World再接続時はpending Taskを`task_update phase=queued`として再通知する。Core再起動時にconsult中だった`active` pre-Agent TaskはFIFO先頭へ戻すが、同じ`task_id`のAgent Sessionまたは`runtime/workspace/<task_id>/task_result.json`の終端結果が既にDurable化されていれば再実行しない。pre-Agent終端結果はQueue解放やUI送信より先に保存し、ownerが残る未配送結果はWorld再接続時に既存outboxへ再送する。QueueとAgent Snapshotは所属`workflow_id`を保持し、Workflow完了判断はそれらの現在状態を使う。所属Workflowが完了/解決済みなら復旧・Queueからの再実行を開始しない
