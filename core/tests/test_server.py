...
def test_brain_provider_list_exposes_codex_and_future_unavailable_providers(tmp_path: Path) -> None:
    async def scenario() -> None:
        server = CoreServer(_make_config(tmp_path), port_override=0)
...
def test_resident_create_requires_empty_m1_slot_and_returns_updated_settings(tmp_path: Path) -> None:
    async def scenario() -> None:
        server = CoreServer(_make_config(tmp_path), port_override=0)
        server._provider_is_available = lambda provider: provider == "codex"  # type: ignore[method-assign]
        await server.start()
        try:
            ...
            async with connect(f"ws://127.0.0.1:{port}") as websocket:
                ...
                assert response["id"] == "resident-create-1"
                ...
                assert server.resident_service.enabled_names == ("Kina",)
        finally:
            await server.stop()
    ...
    async def scenario() -> None:
        ...
        try:
            ...
            async with connect(f"ws://127.0.0.1:{port}") as websocket:
                ...
                assert response["payload"]["resident"]["brain"] == "codex"
                assert server.resident_service.load("Kina").brain == "codex"
        finally:
            await server.stop()
