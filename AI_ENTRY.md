...
- **Holo Addon**：Gate 0完了、Avatar統合済み、Conversation Runtime実装済み
  ...
  - Holo Local Clientから通常Taskを開始・状態取得・待機・Cancel・Recovery・非特権Question応答できる。別Task Runtimeは持たず、Worldと同じTask Queue / Agent Runtime / `tasks.allowed_dirs`安全境界を使う。反復作業は`task-start`→`task-wait`を標準経路とし、`waiting_for_master`のQuestionでは`task-respond`、Approval / Plan決裁ではMaster UI、Core crash後の`interrupted`では`task-recover`へ分岐する
